-- Reservation-decay tracking (#8): decay_state_at stamps when the current
-- decay_state was entered (drives the owner->reserver grace window and the
-- expire window); decay_release_token_hash holds the hash of the single-purpose
-- one-click release token minted at reserver_notified (empty = none).
-- Existing rows backfill decay_state_at from last_activity_at.
